...
절차·정책·응대·운영 기준 질문은 DB 조회 대상이 아니다. `openknowl-data:miniintern-operations` 스킬을 사용한다.

- "수료 기준이 뭐야?", "기업이 연장 요청하면 어떻게 처리해?", "참가자에게 뭐라고 답해?" → 운영 지식 스킬
- 수치와 기준이 섞인 질문이면 수치만 이 스킬에서 조회하고, 기준·절차는 운영 지식 스킬로 답한다.
