...
>
> Claude/Cowork 셋업 순서:
> 1. 관리자에게 **`openknowl-credentials.json` 파일**을 안전한 채널로 받으세요.
> 2. 본인 컴퓨터에 폴더 하나 만들고 (예: `문서/오픈놀`), 받은 파일을 그대로 그 안에 넣으세요.
> 3. Cowork 앱 왼쪽 사이드바에서 **Projects** 를 열고 → **새 프로젝트 생성** → 방금 만든 폴더를 선택하세요.
> 4. 그 프로젝트 안에서 대화를 시작하면 파일을 자동으로 찾아 사용해요.
>
> 파일을 받을 수 없고 토큰만 받은 경우에는 터미널에서 아래 명령을 한 번 실행하세요. 토큰은 입력 중 화면에 표시되지 않고, 파일은 현재 폴더에 안전하게 저장됩니다.
>
> ```bash
> python3 - <<'PY'
> import getpass, json, os
> token = getpass.getpass("관리자 토큰: ")
> with open("openknowl-credentials.json", "w") as f:
>     json.dump({"PROXY_TOKEN": token}, f)
>     f.write("\n")
> os.chmod("openknowl-credentials.json", 0o600)
> print("저장 완료")
> PY
> ```
>
> Codex에서는 토큰 파일을 넣은 작업 폴더를 열고 대화를 시작하면 준비 완료입니다.
>
